...
export default async function calculateRouteExposureMapbox(route) {
  ...
  for (let j = 0; j < steps.length; j++) {
    ...
    if (steps[j].distance < 1000) {
      ...
      let chunkLength = Math.floor(steps[j].geometry.coordinates.length / chunks); // number of coordinates in each chunk
  ...
  for (let j = 0; j < routePoints.length; j++) {
    //fetch the aqi values for the points in the routes
    try {
      if (routePoints[j] === undefined || routePointTime[j] === undefined) {
        continue;
      }
      const aqiData = await fetchAqiData([routePoints[j][0], routePoints[j][1]]);
      console.log({ aqiData });
      totalRouteExposure = totalRouteExposure + (aqiData.data.iaqi.pm25.v * routePointTime[j]) / 3600; // converting the time to hours
      console.log('The total exposure for the route is: ', totalRouteExposure);
      console.log('The pm2.5 from mapbox concentration is ', aqiData.data.iaqi.pm25.v);
