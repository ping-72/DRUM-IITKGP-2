...
// import ParametersNormalizedValues from '../utils/GetNormalizedValues.jsx';
...
export default function MapDrawer() {
  ...
  function addMarkerToMap({ position, placeName, locationType = 'default' }) {
    ...
    new mapboxgl.Popup().setLngLat(position).setHTML(`<h3 color: "black"><strong>${placeName}</strong></h3>`).addTo(window.$map);
  ...
  async function getAllRoutes(start, end) {
    ...
    if (temp_mode === 'foot' || temp_mode === 'bike' || temp_mode === 'scooter') {
      routes = await getGraphhopperRoutes(temp_mode, source, destination);
    } else {
      routes = await getMapboxRoutes(source, destination);
    }
  ...
  return (
    <div className="drawer">
      {/* Two buttons - One for getting Location */}
      <input
        id="my-drawer"
        type="checkbox"
      ...
      <div className="drawer-content">
        ...
        {/* Button for the ZoomIn or ZoomOut */}
        <div>
